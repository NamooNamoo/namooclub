							...
							<div class="tab-pane fade active in" id="joined">
								...
								<ul class="list-group">
									<c:forEach var="myCommunity" items="${my_community_list}">
										...
										<span class="badge">${myCommunity.date}</span>
											<h4>
												<a
													href="${ctx}/community/main.do?community_name=${myCommunity.id}">${myCommunity.name} (클럽:${myCommunity.getAllClub().size()}) (회원:${myCommunity.members.size()})</a>
											</h4>
											<p>${myCommunity.description}</p> <c:choose>
												<c:when test="${myCommunity.manager.email ne loginId}">
													<button type="button" class="btn btn-default btn-sm"
														onclick="location.href='${ctx}/view/community/withdrawal.xhtml?community_name=${myCommunity.id}'">멤버탈퇴
														신청하기</button>
												</c:when>
							...
							<div class="tab-pane fade" id="unjoinded">
								...
								<ul class="list-group">
									<c:forEach var="notMyCommunity"
										...
										<li class="list-group-item">
										<span class="badge">${notMyCommunity.date}</span>
											<h4>
												<!-- <span class="label label-info">추천</span>&nbsp; -->
												<a
													href="${ctx}/community/main.do?community_name=${notMyCommunity.id}&join=false">${notMyCommunity.name} (클럽:${notMyCommunity.getAllClub().size()}) (회원:${notMyCommunity.members.size()})</a>
											</h4>
											<p>${notMyCommunity.description}</p>
											<button type="button" class="btn btn-default btn-sm"
												onclick="location.href='${ctx}/view/community/join.xhtml?community_name=${notMyCommunity.id}'">멤버
												가입하기</button>
										</li>
									</c:forEach>
								</ul>
							...
							<div>
								...
								<ul class="list-group">
									<c:forEach var="community" items="${community_list}">
										<li class="list-group-item">
										<span class="badge">${community.date}</span>
											<h4>
												<a
													href="${ctx}/community/main.do?community_name=${community.id}">${community.name} (클럽:${community.getAllClub().size()}) (회원:${community.members.size()})</a>
											</h4>
